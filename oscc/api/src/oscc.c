...
static int epas_socket = -1;

#define OSCC_MAGIC_EPOS_BYTE_0 ( 0x20 ) //Mode byte 20 for EPS angle messages
#define OSCC_MAGIC_EPOS_BYTE_1 ( 0x00 ) // Reserved 00
#define OSCC_MAGIC_EPOS_SPEED_BYTE_0 ( 0x64 ) // Speed 64 for 100 and FA for 250
#define OSCC_MAGIC_EPOS_SPEED_BYTE_1 ( 0x00 )  //need to be checksumm XOR
#define CAN_EFF_MASK 0x1FFFFFFFU
#define OSCC_STEERING_COMMAND_CAN_ID 0x469
...
oscc_result_t epas_open( unsigned int channel )
{
    oscc_result_t result = OSCC_ERROR;
    ...
    printf( "Opening EPAS CAN channel: %s\n", can_string_buffer );

    result = epas_init_can( can_string_buffer );
...
oscc_result_t oscc_publish_throttle_position( double throttle_position )
{
    ...
    // printf( "Writing to socket: %x, %p, %zu\n", OSCC_THROTTLE_COMMAND_CAN_ID, (void *)&throttle_cmd, sizeof(throttle_cmd));
...
oscc_result_t oscc_publish_epos_value( long steering_pos )
{
    ...
    unsigned char* steering_hex = (unsigned char *)&steering_pos;

    oscc_steering_epos_s epos_cmd = 
    {
        .magic[0] = ( uint8_t ) OSCC_MAGIC_EPOS_BYTE_0,
        .magic[1] = ( uint8_t ) OSCC_MAGIC_EPOS_BYTE_1,
        .epos_speed_values[0] = ( uint8_t ) OSCC_MAGIC_EPOS_SPEED_BYTE_0,
        .epos_speed_values[1] = ( uint8_t ) OSCC_MAGIC_EPOS_SPEED_BYTE_1
    };

    epos_cmd.epos_pos[0] = steering_hex[0];
    epos_cmd.epos_pos[1] = steering_hex[1];
    epos_cmd.epos_pos[2] = steering_hex[2];
    epos_cmd.epos_pos[3] = steering_hex[3];

    result = epos_can_write(
...
oscc_result_t oscc_disable_steering( void )
{
    ...
    {
        .magic[0] = ( uint8_t ) OSCC_DRIVE_0,
        .magic[1] = ( uint8_t ) OSCC_DRIVE_0
    };

    result = oscc_can_write(
        ( uint32_t )OSCC_STEERING_DISABLE_CAN_ID,
...
oscc_result_t epos_can_write( long id, void *msg, unsigned int dlc )
{
    oscc_result_t result = OSCC_ERROR;
    ...
    if ( epas_socket != -1 )
    {
        ...
        char can_string_buffer[32];

        snprintf( can_string_buffer, 32, "can%u", 0 );


        struct ifreq ifr;
        memset( &ifr, 0, sizeof(ifr) );

        strncpy( ifr.ifr_name, can_string_buffer, IFNAMSIZ );

        int some_what = ioctl( epas_socket, SIOCGIFINDEX, &ifr );
        ...
        // int socket_options = setsockopt(can_socket, SOL_CAN_RAW, CAN_RAW_FILTER, &rfilter, sizeof(rfilter));
        ...
        // rfilter[0].can_id   = 0x18FF00F9;
        // rfilter[0].can_mask = CAN_EFF_MASK;

        // setsockopt(can_socket, SOL_CAN_RAW, CAN_RAW_FILTER, &rfilter, sizeof(rfilter));


        if (bind(epas_socket, (struct sockaddr *) &can_address, sizeof(can_address) ) < 0) {
            perror("bind");
            return 1;
        }

        int ret = write( epas_socket, &tx_frame, sizeof(tx_frame) );
...
oscc_result_t oscc_init_can( const char *can_channel )
{
    ...
    // rfilter[0].can_id   = OSCC_STEERING_COMMAND_CAN_ID;
    // rfilter[0].can_mask = CAN_EFF_MASK;
    // // setsockopt(sock, SOCK_RAW, CAN_RAW, &rfilter, sizeof(rfilter));
    // int socket_options = setsockopt(sock, SOL_CAN_RAW, CAN_RAW_FILTER, &rfilter, sizeof(rfilter));
    if ( sock < 0 )
    {
...
oscc_result_t epas_init_can( const char *can_channel )
{
    int result = OSCC_ERROR;
    int ret = -1;
    ...
    //     snprintf( can_string_buffer, 16, "can%u", 0 );
    //     snprintf( can_string_buffer, 16, "can%u", 1 );
    ...
    //     int some_ret = ioctl( can_socket, SIOCGIFINDEX, &ifr );
